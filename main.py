...
# Config 
class Config(object):
    BOT_TOKEN = os.environ.get("BOT_TOKEN", "7335803040:AAGrGzfz7k6hgcYXFNKbq5WCm1XeJ1bYuaI")
    API_ID = int(os.environ.get("API_ID",  "5360874"))
    API_HASH = os.environ.get("API_HASH", "4631f40a1b26c2759bf1be4aff1df710")
    DOWNLOAD_LOCATION = "./DOWNLOADS"
    SESSIONS = "./SESSIONS"

    AUTH_USERS = os.environ.get('AUTH_USERS', '1195351595,6324457826').split(',')
    for i in range(len(AUTH_USERS)):
        AUTH_USERS[i] = int(AUTH_USERS[i])

    GROUPS = os.environ.get('GROUPS', '-4505450960').split(',')
    for i in range(len(GROUPS)):
        GROUPS[i] = int(GROUPS[i])

    LOG_CH = os.environ.get("LOG_CH", "1195351595")
